...
namespace MoonyRepair {
	...
	public class JobDriver_Repair : JobDriver {
		...
		protected override IEnumerable<Toil> MakeNewToils () {
			...
			repair.tickAction = delegate () {
				...
				float num = actor.GetStatValue (StatDefOf.GeneralLaborSpeed, true) * 1f;
	...
	public class RepairUtility {
		...
		public static bool PawnCanRepairNow (Pawn pawn, Thing t) {
			...
			ans &= ((float) t.HitPoints/ (float) t.MaxHitPoints) < settings.repairThreshold;
			
			return ans;
		}

		public static bool PawnCanRepairEver (Thing t) {
			RepairSettings settings = LoadedModManager.GetMod<RepairMod> ().GetSettings<RepairSettings> ();
	...
	public class RepairMod : Mod {
		...
		public override void DoSettingsWindowContents (Rect inRect) {
			...
			listingStandard.Gap ();
			listingStandard.LabelDouble ("Damage Threshold:", settings.repairThreshold.ToString(), "Items below this percentage threshold will be valid targets.");
